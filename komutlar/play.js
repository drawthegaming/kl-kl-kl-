...
exports.run = async (client, message, args) => {
    ...
    .setDescription(".play (İsim Veya Link)")
    ...
    if (!permissions.has('CONNECT')) {
      ...
      .setColor("BLACK")
      .setDescription(`<a:hypesqu:793475343595077705>Ses Kanalına Girmek İçin Yetkim Yok.`)
      return message.channel.send(warningErr);
    }
    if (!permissions.has('SPEAK')) {
      const musicErr = new RichEmbed()
      .setColor("BLACK")
      .setDescription(`<a:hypesqu:793475343595077705>Mikrofonum Kapalı Açmazsan Şarkı Çalamam!.`)
      ...
      try {
          ...
          .setColor("BLACK")
          .setDescription(`<a:hypesqu:793475343595077705>Böyle Bir Şarkı Bulamadım`) 
    ...
    async function handleVideo(video, message, voiceChannel, playlist = false) {
        ...
        if (!serverQueue) {
          ...
          try {
            ...
            console.error(`<a:hypesqu:793475343595077705>Ses Kanalına Giremedim : ${error}`);
            queue.delete(message.guild.id);
            return message.channel.send(`<a:hypesqu:793475343595077705>Ses Kanalına Giremedim HATA: ${error}`);
          }
        } else {
        ...
        .addField("Şarkıyı Seçen Kişi", `${song.requester}`, true)
